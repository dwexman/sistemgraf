...
export default function Contacto() {
  ...
  try {
    const res = await fetch("https://sistemgraf.cl/api/contact.php", {
      method: "POST",
      body: formData,
    });

    const raw = await res.text();         // 👈 leemos texto crudo SIEMPRE
    ...
    if (!res.ok || !data?.ok) {
      ...
      // muestra el toast de error con lo que realmente devolvió el servidor
      showToast(`Error: ${msg}`, "error");
    } else {
      showToast("¡Gracias! Tu mensaje fue enviado con éxito.", "success");
      formEl.reset();
    }
  } catch (err) {
    console.error("fetch error:", err);
    showToast("Error de red. Ver consola.", "error");
  } finally {
    setSending(false);
  }
};

  // ---- THREE.js (igual que tu código)
  useEffect(() => {
    ...
    const tick = () => {
      ...
      for (let i = 0; i < particlesCount; i++) {
        const i3 = i * 3,
          x = particlesGeometry.attributes.position.array[i3];
  ...
  return (
    <section
      id="contacto"
      className="bg-[#EFEEF5] py-16 sm:py-20 flex items-center justify-center min-h-screen relative"
    >
      <canvas
        ref={canvasRef}
      ...
      <style>{`
        ...
        .underline-wrap { position: relative; display: inline-block; padding: 0 .08em .26em; white-space: normal; }
        .u-text { position: relative; z-index: 2; }
        .underline-bar {
          position: absolute; left: 0; right: 0; bottom: 0; height: .16em;
          border-radius: .22em; background: linear-gradient(90deg,#00A3E0 0%, #69A9D1 50%, #005587 100%);
          transform-origin: 0 50%; transform: scaleX(0);
          animation: underlineGrow 900ms cubic-bezier(.2,.9,.2,1) 120ms forwards; opacity: .95; z-index: 0;
        }
        .underline-shimmer { position: absolute; left: 0; right: 0; bottom: 0; height: .16em; border-radius: .22em; overflow: hidden; pointer-events: none; z-index: 1; }
      ...
      {toast.show && (
        <div
          ...
          aria-live="polite"
          className={`
            fixed left-1/2 -translate-x-1/2 bottom-6 z-50 px-5 py-3 rounded-xl shadow-xl
      ...
      <div className="w-full relative z-10">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 mb-10 sm:mb-12 text-center floaty">
          <h3 className="text-[#0A2F4F] font-extrabold leading-tight text-[24px] sm:text-[30px] md:text-[36px]">
            ¿Listo para{" "}
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#005587] to-[#00A3E0]">
              potenciar
            </span>{" "}
        ...
        <div
          className="mx-auto max-w-md px-4 sm:px-6 lg:px-8 flex justify-center items-center relative"
          style={{ zIndex: 2 }}
        >
          <div
            className="
